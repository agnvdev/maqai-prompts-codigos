...
## INP Achievement Rates

As of late 2025:
- **77% of mobile pages** now achieve "good" INP (≤200ms) — up from **55% in 2022** when INP was first introduced
- Desktop INP pass rate: **96%**
- INP is now the most commonly passed CWV metric on mobile

## HTTP/3 & QUIC Protocol

HTTP/3 reached **35% global adoption** (late 2025), with significant performance implications:

| Metric | HTTP/2 | HTTP/3 | Improvement |
|--------|--------|--------|-------------|
| Mobile latency | Baseline | -30% | Reduced round trips via QUIC |
| Connection setup | TCP + TLS (3 RTT) | 0-RTT QUIC | Faster first byte |
| Head-of-line blocking | Yes (TCP) | No (per-stream) | Better multiplexing |
| Connection migration | No | Yes | Better mobile experience |

### CDN HTTP/3 Support (2025)
| CDN | HTTP/3 | Notes |
|-----|--------|-------|
| Cloudflare | Default | All plans, automatic |
| AWS CloudFront | Opt-in | Enabled via distribution settings |
| Fastly | Yes | QUIC support since 2024 |
| Akamai | Yes | HTTP/3 with QUIC |
| Vercel | Yes | Via edge network |

### Detection
```bash
# Check HTTP/3 support
curl -sI --http3 https://example.com 2>&1 | head -1

# Alternative: check via headers
curl -sI https://example.com | grep -i "alt-svc"
# Look for: alt-svc: h3=":443"
```

**Recommendation:** Enable HTTP/3 for all sites. Zero risk (falls back to HTTP/2), significant mobile performance gains.
